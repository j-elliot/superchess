...
//Core concept: In chess, the king is the only piece that moves. It can move itself or another piece. In super chess, this can be expanded to other pieces such as the magistrate, chancellor, and Lord, each of which get their own turn. The Lord can move other pieces, the magistrate can perform certain other functions (such as cards, if available) the chancellor can only move itself. So it's defined by an int
//0 is normal piece
//2 is moves itself
//3 is plays cards
